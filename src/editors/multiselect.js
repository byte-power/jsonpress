...
export class MultiSelectEditor extends AbstractEditor {
    ...
    getNumColumns() {
        ...
        Object.keys(this.select_values).forEach(
            i => (longestText = Math.max(longestText, `${this.select_values[i]}`.length + 4))
        );
    ...
    build() {
        ...
        if ((!this.schema.format && this.option_keys.length < 8) || this.schema.format === 'checkbox') {
            ...
            this.theme.setSelectOptions(
                this.input,
                this.option_keys,
                this.option_enum.map(e => e.title)
            );
        ...
        this.multiselectChangeHandler = e => {
            ...
            for (i = 0; i < this.option_keys.length; i++) {
                if (
                    this.select_options[this.option_keys[i]] &&
                    (this.select_options[this.option_keys[i]].selected ||
                        this.select_options[this.option_keys[i]].checked)
                )

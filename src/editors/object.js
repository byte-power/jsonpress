...
export class ObjectEditor extends AbstractEditor {
        ...
        if (this.format === 'grid-strict') {
            ...
            if (isCategoriesFormat) {
                ...
                this.property_order.forEach(key => {
                    ...
                    const isObjOrArray =
                        editor.schema && (editor.schema.type === 'object' || editor.schema.type === 'array')
                    /* mark the pane */
    ...
    preBuild() {
        ...
        if (this.options.table_row) {
            Object.entries(this.schema.properties).forEach(([key, schema]) => {
                ...
                this.editors[key] = this.jsoneditor.createEditor(
                    editor,
                    {
                ...
                const width = this.editors[key].options.hidden
                    ? 0
                    : this.editors[key].options.grid_columns || this.editors[key].getNumColumns()
            ...
            if (!this.schema.defaultProperties) {
                if (this.jsoneditor.options.display_required_only || this.options.display_required_only) {
                    this.schema.defaultProperties = Object.keys(this.schema.properties).filter(k =>
                        this.isRequiredObject({ key: k, schema: this.schema.properties[k] })
                    )
                } else {
    ...
    addTab(idx) {
        const isObjOrArray =
            this.rows[idx].schema && (this.rows[idx].schema.type === 'object' || this.rows[idx].schema.type === 'array')
        if (this.tabs_holder) {
            this.rows[idx].tab_text = document.createElement('span')

            if (!isObjOrArray) {
                this.rows[idx].tab_text.textContent =
                    typeof this.schema.basicCategoryTitle === 'undefined' ? 'Basic' : this.schema.basicCategoryTitle
            } else {
                const isCategoriesFormat = this.format === 'categories'
                this.rows[idx].tab_text.textContent = this.rows[idx].getHeaderText(isCategoriesFormat)
            }
            this.rows[idx].tab = this.theme.getTopTab(
                this.rows[idx].tab_text,
                this.getValidId(this.rows[idx].tab_text.textContent)
            )
    ...
    showAddProperty() {
        ...
        if (targetRect.top + this.addproperty_holder.offsetHeight > window.innerHeight) {
            ...
            this.addproperty_holder.style.top = `${
                this.addproperty_button.offsetTop + this.addproperty_button.offsetHeight
            }px`
        }
        ...
        if (this.cached_editors[name]) {
            ...
            if (
                ...
                (!this.schema.patternProperties ||
                    !Object.keys(this.schema.patternProperties).find(i => new RegExp(i).test(name)))
            ) {
                return
            }
            ...
            this.editors[name] = this.jsoneditor.createEditor(
                editor,
                {
                    jsoneditor: this.jsoneditor,
    ...
    refreshAddProperties() {
        if (
            ...
            (this.options.disable_properties !== false && this.jsoneditor.options.disable_properties)
        ) {
        ...
        canAdd =
            this.canHaveAdditionalProperties() &&
            !(typeof this.schema.maxProperties !== 'undefined' && numProps >= this.schema.maxProperties)

            ...
            extend(
                {},
                {
                    ...
                    upload_handler: (type, file, cbs) => {
                        ...
                        window.alert(
                            `No upload_handler defined for "${this.path}". You must create your own handler to enable upload to server`
                        )
                    }
                },
        ...
        if (!this.schema.readOnly && !this.schema.readonly) {
                ...
                if (files && files.length) {
                    if (this.options.max_upload_size !== 0 && files[0].size > this.options.max_upload_size) {
                        this.theme.addInputError(
                            this.uploader,
                            `Filesize too large. Max size is ${this.options.max_upload_size}`
                        )
                    } else if (
                        this.options.mime_type.length !== 0 &&
                        !this.isValidMimeType(files[0].type, this.options.mime_type)
                    ) {
                        this.theme.addInputError(
                            this.uploader,
                            `Wrong file format. Allowed format(s): ${this.options.mime_type.toString()}`
                        )
                    } else {
            ...
            this.dragHandler = e => {
                ...
                const validType =
                    files &&
                    files.length &&
                    (this.options.mime_type.length === 0 || this.isValidMimeType(files[0].type, this.options.mime_type))
                const validZone =
                    e.currentTarget.classList && e.currentTarget.classList.contains('upload-dropzone') && validType

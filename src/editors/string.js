...
export class StringEditor extends AbstractEditor {
    ...
    getNumColumns() {
        const min = Math.ceil(
            Math.max(this.getTitle().length, this.schema.maxLength || 0, this.schema.minLength || 0) / 5
        )
        let num
    ...
    build() {
        ...
        let { input } = this
        if (this.format === 'range') {
            input = this.theme.getRangeControl(
                this.input,
                this.theme.getRangeOutput(this.input, this.schema.default || Math.max(this.schema.minimum || 0, 0))
            )
        }
    ...
    setupCleave(el) {
        ...
        const options = this.expandCallbacks(
            'cleave',
            extend({}, this.defaults.options.cleave || {}, this.options.cleave || {})
        )
    ...
    setupImask(el) {
        ...
        const options = this.expandCallbacks(
            'imask',
            extend({}, this.defaults.options.imask || {}, this.options.imask || {})
        )
        if (typeof options === 'object' && Object.keys(options).length > 0) {

...
export class AbstractEditor {
    constructor(options, defaults) {
        ...
        this.key =
            this.parent !== undefined
                ? this.path.split('.').slice(this.parent.path.split('.').length).join('.')
                : this.path;
    ...
    addLinks() {
        ...
        if (!this.no_link_holder) {
            ...
            /* otherwise just insert link at bottom of container */ else {
                if (this.control) {
    ...
    updateHeaderText() {
        let titleOnly =
            ...
            (this.schema.type === 'array' && !['select2', 'checkbox'].includes(this.schema.format)) ||
            (this.schema.type === 'boolean' && this.schema.format === 'checkbox');
        if (this.header) {
    ...
    destroy() {
        ...
        if (this.watched) {
            Object.values(this.watched).forEach(adjustedPath =>
                this.jsoneditor.unwatch(adjustedPath, this.watch_listener)
            );
        }
    ...
    isRequired() {
        ...
        else if (this.realParent && this.realParent.schema && Array.isArray(this.realParent.schema.required))
            return this.realParent.schema.required.includes(this.key);
    ...
    getDisplayText(arr) {
        ...
        arr.forEach(el => {
            ...
            /* Object */ else if (el.title && used[el.title] <= 1) name = el.title;
